...
func extractFieldsStacksToBuff(buff zapBuffer, fields []zapcore.Field) []zapcore.Field {
    ...
    for i, field := range fields {
        ...
        oldFields := fields
        fields = make([]zapcore.Field, len(fields)-1)
        if len(oldFields) > 1 {
            fields = append(oldFields[0:i], oldFields[i+1:]...)
        }

        if buff.Len() != 0 {
            buff.AppendByte('\n')
        }
        if stackedFmt, ok := stacked.(fmt.Formatter); ok {
            buff.AppendString("Error: ")
            stackedFmt.Format(zapBufferFmtState{buff}, 'v')
            //appendStack(buff, field.Key, stackedFmt)
        } else {
            buff.AppendString("Stacktrace: ")
            stacked.StackTrace().Format(zapBufferFmtState{buff}, 'v')
...
func appendStack(buff zapBuffer, key string, stack fmt.Formatter) {
    if buff.Len() != 0 {
        buff.AppendByte('\n')
    }
